'''
part of the code is from https://github.com/HuCaoFighting/Swin-Unet?tab=readme-ov-file
...
class SwinTransformerBlock(nn.Module):
    ...
    Args:
        ...
        act_layer (nn.Module, optional): Activation layer. Default: nn.SELU
...
class PatchEmbed(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        # print(f'degbug{img_size}, {patch_size}')
...
class SwinTransformerSys(nn.Module):
    ...
    def __init__(
        ...
        ** kwargs,
        ...
        if self.VAE:
            self.feature_size = (img_size/(2**(self.num_layers+1)))**2
            self.fc_mu = nn.Linear(
                int(self.num_features*self.feature_size), self.latent_dim)
            self.fc_var = nn.Linear(
                int(self.num_features*self.feature_size), self.latent_dim)
            self.decode_input = nn.Linear(self.latent_dim, int(
        ...
        if self.final_upsample == "expand_first":
            ...
            self.output1 = nn.Sequential(block_klass(embed_dim, embed_dim//2, time_emb_dim = None),nn.Conv2d(embed_dim//2, self.num_classes, 1))

        self.apply(self._init_weights)

    def _init_weights(self, m):
        if isinstance(m, nn.Linear):
            trunc_normal_(m.weight, std=0.05)
    ...
    def forward_up_features(self, x, x_downsample):
        # print(f"decode input shape is {x.shape}")
        for inx, layer_up in enumerate(self.layers_up):
            if inx == 0:
                x = layer_up(x)
                # print(f"decode step {inx} shape {x.shape}")
            else:
                # # print(
                #     f"x with shape {x.shape} concat with encoder feature with shape {x_downsample[3-inx].shape}"
                # )
    ...
    def up_x4(self, x):
        ...
        if self.final_upsample == "expand_first":
            ...
            # print(f"x shape {x.shape} before output")
            # x = self.output(x)
            x = self.output1(x)
    ...
    def forward(self, x):
        if self.Fourier:
            ...
            x = torch.view_as_real(x)  # B,H,W,2
            x = x.permute(0, 3, 1, 2).contiguous().log()
